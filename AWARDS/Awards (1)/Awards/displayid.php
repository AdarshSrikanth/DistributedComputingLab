<?php
	...
	$sql="SELECT * FROM detail WHERE eid='$var'";
 ...
 fputs($file,$s) or die("Data not written ");
   ...
   if(!$result1)
   {
	...
	$query1="SELECT nature from detail where eid='$var'";
   $result1=$conn->query($query1);
   if(!$result1)
   {
	echo 'could not run query: '.mysql_error();
	...
	$query1="SELECT title from detail  where eid='$var'";
   $result1=$conn->query($query1);
   if(!$result1)
   {
	echo 'could not run query: '.mysql_error();
	...
	$title=$arrayresult1['title'];
		...
		$query1="SELECT year from detail where eid='$var'";
   $result1=$conn->query($query1);
   if(!$result1)
   {
	echo 'could not run query: '.mysql_error();
